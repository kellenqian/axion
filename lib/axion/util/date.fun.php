<?php
	...
	***************************************************/ 
	 ...
	 * @return array
	 */
	function cutDateMonth( $arr_date, $int_months )
	{
		//验证日期合法性
		if( !checkInt( $int_months, 12, 1 ) || !checkInt( $arr_date['m'], 12, 1 ) )
			return false;
	 ...
	 * @return integer
	 */
	function getDaysInMonth( $int_year, $int_month )
	{
		//验证日期合法性
		if( !checkInt( $int_month, 12, 1 ) )
			return false;
	 ...
	 */
	function getSubDay( $arr_startTime, $arr_endTime='' )
	{
		//合集日期差
		$_int_days = 0;
	 ...
	 */
	function getSubMonth( $arr_startTime, $arr_endTime )
	{
		//验证日期合法性
		if( !checkInt( $arr_startTime['m'], 12, 1 ) )
			return false;
			
		if( empty( $arr_endTime ) )
			$arr_endTime = @getdate();
		//验证日期合法性
		else if( !checkInt( $arr_endTime, 12, 1 ) )
			return false;
