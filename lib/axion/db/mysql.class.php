<?php
	...
	class Axion_db_MySQL extends Axion_db_Database
	{
		 ...
		 * @return true
		 */
		public function changeDB( $str_databaseName = null, $str_user = null, $str_password = null )
		{
			...
			/**
			 * 如果未提供新的账号但提供了新的数据库名称则使用当前账号重新选择数据库
			 */
			elseif( !is_null( $str_databaseName ) )
			{
				if( !empty( $this->hd_connect ) )
				{
					if( !@mysql_selectdb( $this->str_db, $this->hd_connect ) )
		...
		public function getLastInsertID()
		{
			...
			if( !$_void_resutl )
			{
				$this->obj_Axion_log->newMessage( '尝试获取最后一次插入的记录的主键值失败。', Axion_log::NOTICE );
				return false;
			} //if
