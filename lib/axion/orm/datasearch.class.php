...
class Axion_orm_DataSearch {
	...
	function __construct() {
		...
		if (! class_exists ( $this->str_tableName )) {
			if (! file_exists ( DB_CACHE_PATH . DS ."{$this->str_tableName}.class.php" )) {
				$_obj_initDataMap = new Axion_orm_InitDataMap ( $this->str_tableName );
				$_obj_initDataMap->createFile ( DB_CACHE_PATH );
			}
			
			require_once (DB_CACHE_PATH . DS ."{$this->str_tableName}.class.php");
		}
